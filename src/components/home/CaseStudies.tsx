...
export function CaseStudies({ projects: dbProjects }: { projects?: any[] }) {
  ...
  // Map database project schema to frontend schema if projects are passed from parent
  const projectsList = dbProjects && dbProjects.length > 0
    ? dbProjects.map(mapDbProjectToProjectDetails)
    : projects;

  // Focus on NexDrop and Taskip as our highly featured case studies
  const featured = projectsList.filter(p => p.id === "nexdrop" || p.id === "taskip");

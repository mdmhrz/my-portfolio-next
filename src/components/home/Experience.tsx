...
export function Experience({ experiences: dbExperiences }: { experiences?: any[] }) {
  const [openId, setOpenId] = useState<string | null>(null);

  const hasDb = dbExperiences && dbExperiences.length > 0;

  // Collect all projects for the details modal
  const allProjectsMapped = hasDb
    ? dbExperiences.flatMap((exp: any) => exp.projects || []).map(mapDbProjectToProjectDetails)
    : projects.filter((p) => p.id !== "nexdrop");

  const openProject = allProjectsMapped.find((p) => p.id === openId) || null;
  ...
  return (
    <section id="experience" className="relative overflow-hidden border-t border-border bg-background px-6 py-28 md:py-40">
      <div className="container mx-auto max-w-7xl">
        ...
        {hasDb ? (
          dbExperiences.map((exp: any) => {
            const expProjectsMapped = (exp.projects || []).map(mapDbProjectToProjectDetails);
            return (
              <div key={exp.id} className="mb-14">
                {/* Company card */}
                <Reveal y={40}>
                  <div className="mb-10 overflow-hidden rounded-2xl border border-border bg-card p-7 shadow-sm md:p-10">
                ...
                {/* Projects */}
                {expProjectsMapped.length > 0 && (
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    {expProjectsMapped.map((p: any, idx: number) => (
      ...
      <ProjectDetailsModal
        ...
        onNavigate={(p: any) => setOpenId(p.id)}

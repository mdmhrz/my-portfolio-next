...
export function Journey() {
  return (
    <section id="journey" className="py-40 px-6 relative bg-[#030303] border-y border-white/5">
              ...
              <div className="hidden lg:block space-y-6 pt-10">
                {journey.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-4 group cursor-pointer">
                    <div className={`w-2 h-2 rounded-full transition-all duration-300 ${item.highlight ? "bg-primary scale-150 shadow-[0_0_10px_rgba(20,184,166,0.5)]" : "bg-white/20 group-hover:bg-white/40"}`} />
                    <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${item.highlight ? "text-primary" : "text-white/20 group-hover:text-white/50"}`}>
          ...
          <div className="lg:col-span-8 flex flex-col gap-[10vh] lg:gap-0">
            {journey.map((item, idx) => (
              ...
              >
                ...
                >
                  <div className="text-[10rem] font-black text-white/[0.02] absolute -left-12 -top-12 leading-none select-none pointer-events-none">
                    {item.year.split(" — ")[0]}
                  </div>

                  <div className="glass-card p-8 md:p-12 bg-white/[0.02] hover:bg-white/[0.04] backdrop-blur-3xl shadow-2xl transition-all duration-500 border-white/5 group-hover:border-primary/20">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`text-xs font-bold tracking-tighter transition-colors duration-500 ${item.highlight ? "text-primary" : "text-white/10 group-hover:text-white/40"}`}>
                          {item.year}
                        </span>
                      </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5 flex items-center justify-between">
                      <span className="text-[10px] font-bold text-white/10 uppercase tracking-widest">Engineering Milestone</span>

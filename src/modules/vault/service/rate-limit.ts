import { vaultLoginAttemptsRepo } from "../queries";
...
export async function checkVaultRateLimit(): Promise<{ locked: boolean; retryAfterMinutes?: number }> {
  ...
  const recentFailures = await vaultLoginAttemptsRepo.recentFailures(since, MAX_FAILED_ATTEMPTS);
...
export async function recordVaultAttempt(success: boolean, ipAddress?: string | null): Promise<void> {
  await vaultLoginAttemptsRepo.record(success, ipAddress);
}

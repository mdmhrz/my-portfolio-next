...
import { gmailAccountRepo, threadsRepo, emailMessagesRepo, attachmentsRepo } from "../queries";
...
export async function syncGmailHistory(): Promise<{ processed: number }> {
  ...
  if (!account.historyId) {
    ...
    await gmailAccountRepo.update(account.id, { historyId: profile.data.historyId ?? undefined });
    return { processed: 0 };
  }
  ...
  for (const messageId of addedMessageIds) {
    const existing = await emailMessagesRepo.findByGmailMessageId(messageId);
    ...
    const thread = await threadsRepo.findByGmailThreadId(msg.threadId);
    if (!thread) continue;
    ...
    const emailMessage = await emailMessagesRepo.create({
      threadId: thread.id,
    ...
    await threadsRepo.update(thread.id, { lastMessageAt: new Date(), unread: true });

    processed++;
  }

  await gmailAccountRepo.update(account.id, { historyId: historyRes.data.historyId ?? account.historyId });

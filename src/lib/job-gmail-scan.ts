 ...
 *
 * Deliberately separate from `syncGmailHistory` (src/lib/gmail-sync.ts), which
 * only ever files messages under Threads the app already knows about — job
 * emails come from arbitrary companies with no existing Thread, so they can't
 * share that code path without breaking its "never the whole personal inbox"
...
export async function scanJobGmail(): Promise<{ matched: number; unmatched: number; scanned: number }> {
  ...
  const settings = await prisma.jobTrackerSettings.findUnique({ where: { id: "singleton" } });
  ...
  for (const m of messages) {
    ...
    const [existingEvent, existingUnmatched] = await Promise.all([
      prisma.jobStatusEvent.findUnique({ where: { gmailMessageId: m.id } }),
      prisma.unmatchedJobEmail.findUnique({ where: { gmailMessageId: m.id } }),
    ]);
    ...
    if (job) {
      await prisma.$transaction([
        prisma.jobStatusEvent.create({
          data: { jobId: job.id, status: suggestedStatus, source: "gmail", gmailMessageId: m.id, note: subject || null },
        }),
        prisma.jobApplication.update({
          where: { id: job.id },
          data: {
            status: suggestedStatus,
            gmailThreadId: job.gmailThreadId ?? msg.data.threadId ?? undefined,
          },
        }),
      ]);
      matched++;
    } else {
      await prisma.unmatchedJobEmail.create({
        data: {
          gmailMessageId: m.id,
